...
class MethodAdapter extends MethodVisitor implements Opcodes {
  ...
  public void visitMethodInsn(int opcode, String owner, String name, String desc, boolean itf) {
    if ("java/util/List".equals(owner) && "add".equals(name)) {
      this.visitMethodInsn(
          Opcodes.INVOKESTATIC, "edu/cmu/cs/tot/Library", "start", "()V", false);
      super.visitMethodInsn(opcode, owner, name, desc, itf);
      this.visitMethodInsn(
          Opcodes.INVOKESTATIC, "edu/cmu/cs/tot/Library", "end", "()V", false);
    }
    else {
      // notes: all names are fully qualified. fully qualified names use slashes internally instead
      // of dots.
